  ...
  return (
    <>
            ...
            <Reveal key={key} style={{ textAlign: "left" }}>
              ...
              >
                <span style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  ...
                  <span style={{ width: 22, height: 22, borderRadius: "50%", border: `1px solid ${st.dot}`, display: "flex", alignItems: "center", justifyContent: "center", background: st.dotBg }}>
                    <CheckIcon />
                  </span>
                </span>
      ...
      {path && (
        ...
        >
          <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 12, justifyContent: "space-between", padding: "18px 26px", borderBottom: "1px solid var(--hairline)", background: "var(--elevated)" }}>
          ...
          <div style={{ padding: "clamp(26px,4vw,40px) clamp(20px,3vw,40px)", display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: "clamp(26px,4vw,48px)" }}>
            <div>
              <h3 style={{ fontFamily: "var(--font-space-grotesk)", fontWeight: 600, fontSize: "clamp(23px,2.6vw,31px)", lineHeight: 1.18, letterSpacing: "-.02em", margin: 0 }}>{path.headline}</h3>
              <p style={{ fontSize: 16, lineHeight: 1.65, color: "var(--muted)", margin: "16px 0 0" }}>{path.body}</p>
              <p style={{ fontFamily: "var(--font-jetbrains-mono)", fontSize: 11, letterSpacing: ".18em", color: "var(--muted)", margin: "26px 0 12px" }}>FIRST ENGAGEMENT</p>
              <p style={{ fontSize: 15.5, lineHeight: 1.6, color: "var(--ink)", margin: 0, padding: "16px 18px", borderLeft: "2px solid var(--royal)", background: "rgba(30,79,217,.08)" }}>{path.sprint}</p>
            ...
            <div>
              <p style={{ fontFamily: "var(--font-jetbrains-mono)", fontSize: 11, letterSpacing: ".18em", color: "var(--muted)", margin: "0 0 14px" }}>CAPABILITIES THAT APPLY</p>
              <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                {path.caps.map((cap) => (
                  <Link
                    key={cap.key}
                    href={capabilityRoute(cap.key)}
                    style={{ display: "flex", alignItems: "center", gap: 14, textAlign: "left", borderBottom: "1px solid var(--hairline)", padding: "16px 4px", color: "var(--ink)" }}
                  >
