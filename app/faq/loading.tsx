  ...
  return (
    ...
    >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12, maxWidth: 800, margin: "0 auto" }}>
            {Array.from({ length: 8 }).map((_, i) => (
              <SkeletonCard
                ...
                bodyLines={1}
                delay={500 + i * 60}
                style={{ minHeight: 80, padding: "20px 24px" }}
              />
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: "80px 0", textAlign: "center" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 clamp(20px,4vw,48px)" }}>
          <Skeleton variant="text" width="30%" height="12px" radius="4px" delay={1000} style={{ fontFamily: "var(--font-jetbrains-mono)", fontSize: 12, letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--muted)", border: "none", backgroundColor: "var(--elevated)", margin: "0 auto 16px", display: "inline-block" }} />
          <Skeleton variant="heading" width="60%" height="48px" delay={1100} style={{ fontFamily: "var(--font-space-grotesk)", fontSize: "clamp(2rem,5vw,3.5rem)", fontWeight: 700, lineHeight: 1.08, letterSpacing: "-0.035em", border: "none", backgroundColor: "var(--elevated)", borderRadius: 4, margin: "0 auto 24px" }} />
          <Skeleton variant="text" width="70%" height="24px" delay={1200} style={{ fontSize: 18, lineHeight: 1.55, color: "var(--muted)", border: "none", backgroundColor: "var(--elevated)", borderRadius: 4, margin: "0 auto 32px" }} />
          <Skeleton variant="button" width="auto" delay={1300} style={{ padding: "16px 28px", minHeight: 52 }} />
        </div>
      </section>
    </div>
  );
